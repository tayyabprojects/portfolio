...
const colorStylesList = [
  {
    bg: 'bg-indigo-50 dark:bg-indigo-950/40 text-indigo-650 dark:text-indigo-400 border-indigo-100 dark:border-indigo-900/50',
    badge: 'bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-900/60'
  },
  {
    bg: 'bg-purple-50 dark:bg-purple-950/40 text-purple-650 dark:text-purple-400 border-purple-100 dark:border-purple-900/50',
    badge: 'bg-purple-50 dark:bg-purple-950/30 text-purple-600 dark:text-purple-400 border-purple-200 dark:border-purple-900/60'
  },
  {
  ...
  {
    bg: 'bg-amber-50 dark:bg-amber-950/40 text-amber-655 dark:text-amber-400 border-amber-100 dark:border-amber-900/50',
    badge: 'bg-amber-50 dark:bg-amber-955/30 text-amber-705 dark:text-amber-400 border-amber-200 dark:border-amber-900/60'
  }
];
...
export default function Services({ services }: ServicesProps) {
  return (
    <section id="services" className="py-24 px-4 md:px-8 relative bg-[#f9fafb] dark:bg-[#090b11] transition-colors duration-300">
      ...
      <div className="absolute right-0 bottom-0 w-80 h-80 bg-indigo-600/5 dark:bg-indigo-500/5 blur-[140px] rounded-full pointer-events-none" />

      <div className="max-w-7xl mx-auto">
        
        {/* Section title */}
        <div className="flex flex-col items-center text-center mb-16" id="services-title">
          <h2 className="text-sm font-bold uppercase tracking-widest text-[#4f46e5] dark:text-indigo-400 mb-2 flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#4f46e5] dark:bg-indigo-400"></span> Services Suite
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="services-grid">
          {services.map((srv, idx) => {
            ...
            return (
              <div
                key={idx}
                className="group bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-6 md:p-8 flex flex-col justify-between hover:border-indigo-605/35 dark:hover:border-indigo-500/35 transition-all duration-305 shadow-lg shadow-slate-100 dark:shadow-none relative overflow-hidden"
              >
                {/* Glowing backdrop card effect */}
                <div className="absolute top-0 right-0 w-32 h-32 bg-slate-50 dark:bg-slate-950/45 rounded-bl-full pointer-events-none transition-colors group-hover:bg-indigo-50 dark:group-hover:bg-indigo-950/10" />
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-lg md:text-xl font-extrabold text-slate-900 dark:text-slate-100 group-hover:text-indigo-650 dark:group-hover:text-indigo-400 transition-colors mb-3 tracking-tight">
                    {srv.title}
                  </h3>
                ...
                <div className="flex items-center gap-2.5 pt-4 border-t border-slate-100 dark:border-slate-805 text-xs text-indigo-600 dark:text-indigo-400 font-extrabold transition-colors">
