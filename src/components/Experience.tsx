...
export default function Experience({ experiences }: ExperienceProps) {
  return (
    <section id="experience" className="py-24 px-4 md:px-8 relative bg-slate-50">
      <div className="absolute right-12 top-10 w-96 h-96 bg-purple-600/5 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-center text-center mb-16" id="experience-title">
          <h2 className="text-xs font-bold uppercase tracking-widest text-[#4f46e5] mb-2 flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#4f46e5]"></span> History & Accomplishments
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-12">
          ...
          <div className="lg:col-span-8 space-y-8" id="experience-timeline">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2 font-mono">
              <span className="w-2.5 h-2.5 rounded-full bg-indigo-600 animate-pulse"></span> Commercial Journey
            </h4>
            
            {experiences.map((exp, expIdx) => (
              <div 
                key={expIdx}
                className="bg-white border border-slate-200/80 rounded-3xl p-6 md:p-8 hover:border-indigo-500/20 transition-all duration-300 shadow-lg shadow-slate-100 relative overflow-hidden"
              >
                {/* Floating Period Badge */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                  <div>
                    <span className="text-[10px] md:text-xs font-black text-indigo-600 bg-indigo-50 px-3.5 py-1.5 rounded-full border border-indigo-100 uppercase tracking-wider">
                      {exp.period}
                    </span>
                    <h5 className="text-lg md:text-xl font-black text-slate-900 mt-3 tracking-tight">
                      {exp.role}
                    </h5>
                    <p className="text-indigo-600 font-bold text-xs md:text-sm mt-1">
                      {exp.institution}
                    </p>
                  </div>
                </div>

                <p className="text-slate-650 text-sm leading-relaxed mb-4 italic border-l-2 border-indigo-550/30 pl-3 font-medium">
                  {exp.description}
                </p>
                ...
                <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100">
                  {exp.tags.map((t, idx) => (
                    <span 
                      key={idx} 
                      className="bg-slate-50 text-slate-600 text-[10px] font-bold px-3 py-1.5 rounded-lg border border-slate-250/20 shadow-xs"
                    >
                      {t}
                    </span>
                  ))}
          ...
          <div className="lg:col-span-4 space-y-6" id="achievements-highlights">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2 font-mono">
              <span className="w-2.5 h-2.5 rounded-full bg-purple-600 animate-pulse"></span> core milestones
            </h4>
            ...
            {milestonesList.map((mil, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="bg-white border border-slate-200/80 rounded-3xl p-6 hover:border-purple-500/20 transition-all duration-300 shadow-md shadow-slate-100 relative group"
                >
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center text-purple-600 shrink-0 group-hover:scale-110 transition-transform">
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <h5 className="font-extrabold text-slate-900 group-hover:text-purple-600 transition-colors text-sm md:text-base leading-tight tracking-tight">
                        {mil.title}
                      </h5>
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-3xl p-6 relative overflow-hidden">
              <p className="text-[10px] text-indigo-650 font-black uppercase tracking-widest mb-1 flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping"></span> Status Report
              </p>
              <h5 className="text-lg font-black text-slate-900 tracking-tight">READY TO COLLABORATE</h5>
              <p className="text-xs text-slate-505 mt-2 leading-relaxed font-semibold">
